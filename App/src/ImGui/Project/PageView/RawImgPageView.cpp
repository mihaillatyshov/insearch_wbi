...
namespace LM
{
    ...
    void RawImgPageView::DrawTopMenuExtras()
    {
        ...
        if (ImGui::Button("Действия", buttonSize))
        {
            ImGui::OpenPopup("Меню дейстивий");
        }

        ImGui::SameLine(0.0f, 20.0f);
        if (m_Project->IsPageInGeneratedCatalogExcludePages(m_PageId))
        {
            if (ImGui::Button("Включить в обработку", buttonSize))
            {
        ...
        {
            if (ImGui::Button("Исключить из обработки", buttonSize))
            {
                m_Project->AddGeneratedCatalogExcludePage(m_PageId);
            }
        }

        if (ImGui::BeginPopup("Меню дейстивий"))
        {
            if (ImGui::Selectable("Задать левый верхний паттерн"))
            {
            ...
            if (ImGui::Selectable("Задать правый нижний паттерн"))
            {
            ...
            if (ImGui::Selectable("Просмотр: левый верхний паттерн"))
            {
                m_PatternState = PatternState::kShowTopLeft;
                const CatalogCutPattern& cutPattern = m_Project->GetCatalogTopLeftPattern();
                if (cutPattern.PageId == -1)
                {
                    Overlay::Get()->Start("Паттерн отсутствует");
                }
                else
                {
            ...
            if (ImGui::Selectable("Просмотр: правый нижний паттерн"))
            {
