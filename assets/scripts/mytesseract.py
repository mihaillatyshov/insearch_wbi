...
class TesseractOCR(OCRInstance):
    ...
    def __init__(self, n_threads: int = 1, lang: str = 'eng', psm: int = 11, tessdata_dir: Optional[str] = None):
        ...
        try:
            lang_tess = subprocess.check_output(f"tesseract {self.get_tessdata_dir()} --list-langs",
                                                env=self.env,
                                                shell=True).decode()
    ...
    def get_tessdata_dir(self) -> str:
        if self.tessdata_dir:
            return '--tessdata-dir "' + self.tessdata_dir + '"'
        return ''
    ...
    def hocr(self, image: np.ndarray) -> str:
        ...
        with NamedTemporaryFile(suffix='.jpg', delete=False) as tmp_f:
            ...
            hocr = subprocess.check_output(
                f"tesseract {tmp_file} stdout --oem 1 {self.get_tessdata_dir()} --psm {self.psm} -l {self.lang} hocr",
                env=self.env,
        ...
        for page, hocr in enumerate(content):
            ...
            for element in soup.find_all(class_=True):
                ...
                d_el = {
                    ...
                    "value":
                    re.sub(r"^(\s|\||L|_|;|\*)*$", '', element.string).strip() or None if element.string else None
                }
