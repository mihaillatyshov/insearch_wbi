...
from shared import ArgsBase, openpyxl_ws_autofit, parse_args, get_onedrive_path
...
py_args = Args(xlsx_path=os.path.join(get_onedrive_path(), "Work/wbi/yg1-shop/parsing/out", cmd_args.path,
                                      "3_export_from_toolz.xlsx"),
               save_path=os.path.join(get_onedrive_path(), "Work/wbi/yg1-shop/parsing/out", cmd_args.path))
...
for row in range(2, max_row):
    ...
    for col in range(2, ws_sm.max_column + 1):
        ...
        # если колонок нет в dso заполняем в дополнительнные колонки с заголовками, следущие за основными
            ...
            print("Error of parsing head:", head, "in table:", constr)
            exit(1)
